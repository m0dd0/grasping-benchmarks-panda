...
def transform_grasp_and_offset_to_world(
    grasping_pose: Grasp6D, offset: np.ndarray, camera_pose: CameraData
):
...
def check_collision_between_gripper_and_table(
    grasping_pose: Grasp6D, camera_pose: CameraData, aruco_board_data: ArucoBoardData
):
    ...
    for i in range(len(check_points)):
        ...
        check_point_in_wrf = transform_grasp_and_offset_to_world(
            grasping_pose, check_points[i, :], camera_pose
        )
    ...
    gripper_pose_in_wrf = transform_grasp_and_offset_to_world(
        grasping_pose, [0, 0, 0], camera_pose
    )
...
class GraspNetGraspPlanner(BaseGraspPlanner):
    ...
    def configure(self, cfg: dict):
        ...
        self.cfg_grasp_estimator["num_refine_steps"] = (
            10 if self.cfg_ns.gradient_based_refinement else 20
        )
    ...
    def create_aruco_board_data(
        self, pose: np.ndarray, orientation: np.ndarray, enable_grasp_filter_flag: bool
    ) -> ArucoBoardData:
    ...
    def plan_grasp(
        ...
        self.latest_grasps, self.latest_grasp_scores, output_latents = (
            self.estimator.predict_grasps(
                sess,
                self.object_pc,
                self.estimator.sample_latents(),
                self.cfg_grasp_estimator.num_refine_steps,
            )
        )
        ...
        if len(self.latest_grasps) >= n_candidates:
            sorted_grasps_quality_list = sorted(
                zip(self.latest_grasp_scores, self.latest_grasps),
        ...
        for grasp, score in zip(self.latest_grasps, self.latest_grasp_scores):
            ...
            grasp_6d = Grasp6D(
                position=grasp_with_offset[:3, 3],
            ...
            if aruco_board_data.enable_grasp_filter == True:
                if (
                    check_collision_between_gripper_and_table(
                        grasp_6d, camera_data, aruco_board_data
                    )
                    == False
                ):
    ...
    def visualize(self, visualize_all: bool = True):
        ...
        candidates_to_display = (
            self.n_of_candidates
            if (
                (self.n_of_candidates > 0)
                and (self.n_of_candidates < len(self.latest_grasps))
                and not visualize_all
            )
            else len(self.latest_grasps)
        )
